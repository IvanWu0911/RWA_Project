...
async function main() {
  ...
  const userRes = await client.query(
    "SELECT id, username, wallet_address FROM users WHERE wallet_address IS NOT NULL AND username = 'investor' LIMIT 1;"
  );
  await client.end();

  if (propRes.rows.length === 0) throw new Error("資料庫中無任何已綁定 token_address 的房產！");
  if (userRes.rows.length === 0) throw new Error("找不到用戶 investor 的錢包地址！");
